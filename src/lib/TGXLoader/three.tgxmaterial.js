...
(function() {
  ...
  TGXMaterial.prototype.copy = function(source) {
    ...
    this.detailNormalContributionStrength =
      source.detailNormalContributionStrength;
  ...
  TGXMaterial.prototype.update = function() {
    ...
    switch (this.game) {
      case 'destiny2':
        ...
        uniforms.detailDiffuseTransform = {
          value: this.detailDiffuseTransform
        };
      ...
      default:
        ...
        uniforms.decalAlphaMapTransform = {
          value: this.decalAlphaMapTransform
        };
        uniforms.decalBlendOption = { value: this.decalBlendOption };
        uniforms.detailNormalContributionStrength = {
          value: this.detailNormalContributionStrength
        };
        uniforms.specularProperties = { value: this.specularProperties };

        var spec = this.specularProperties;
        uniforms.specular = {
          value: new THREE.Color(spec[0], spec[0], spec[0])
        };
        uniforms.shininess = { value: spec[1] };
        //uniforms.reflectivity = {value: spec[1]};

        uniforms.subsurfaceScatteringStrength = {
          value: this.subsurfaceScatteringStrength
        };
        break;
    }
    ...
    if (vertexShader.indexOf('USE_DETAIL') == -1) {
      ...
      vertexShader = this.insertAfter(
        ...
        vertexShader,
        chunks.detail_pars_vertex
      );
      vertexShader = this.insertAfter(
        '#include <uv2_vertex>',
        vertexShader,
        chunks.detail_vertex
      );
    }

    if (fragmentShader.indexOf('USE_GEARSTACKMAP') == -1) {
      fragmentShader = this.insertAfter(
        '#include <map_pars_fragment>',
        fragmentShader,
        [chunks.common_pars_fragment, chunks.gearstack_pars_fragment]
      );
  ...
  TGXMaterial.prototype.insertBefore = function(search, shader, insertCode) {
    search += '\n';
    if (typeof insertCode != 'string')
      insertCode = insertCode.join('\n') + '\n';
